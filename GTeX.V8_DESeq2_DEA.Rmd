...
# subject phenotype metadata (this file contains Age and sex information for samples)
...
METADATA <- merge(gtex_subject_meta, brain_gtex_metadata, 
...
hgnc_mapping <- read.delim(url('http://ftp.ebi.ac.uk/pub/databases/genenames/hgnc/archive/monthly/tsv/hgnc_complete_set_2021-06-01.txt'))
...
# subset counts/reads and metadata for the select samples
get_sample_col_names = intersect(c('Name', 'Description', use_samples), colnames(gtex_reads))

brain_gtex_reads = gtex_reads[,get_sample_col_names] # 211 samples

brain_gtex_metadata = gtex_samples %>% filter(SAMPID %in% use_samples) #425 obs
...
brain_gtex_reads = brain_gtex_reads[, -1] # remove description coulmn
...
METADATA = METADATA%>% subset(SAMPID %in% colnames(brain_gtex_reads)) #209
...
coldata$AGE = factor(coldata$AGE, c('20_29', '30_39', '40_49', '50_59', '60_69', '70_79')) # age is set in range in this data
...
# Note: You may choose another age range to serve as the reference level and compare how the results differ
...
dds <- DESeq(dds)
resultsNames(dds) 
GTex.Res.Age <- results(dds, name="AGE_70_79_vs_20_29", alpha = 0.05) # do the same for other age ranges
summary(GTex.Res.Age)
 
```

## Note: you may ggplot to show how the expression of SST gene differs between age groups. 
